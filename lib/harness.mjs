 ...
 *
 * Usage (called by per-app test scripts in tests/<app>/smoke.mjs):
 *   import { runWithFixture } from '../../lib/harness.mjs';
 *   const result = await runWithFixture({ device, fixturePath, instanceName, port, appName, testFn });
 *
 * testFn(port) MUST return { passed, failed } (assertion counts). The harness
 * prints the summary from those counts, sets a non-zero exit code on any
 * failure, and — only when every assertion passed — records the Engine build
 * under compatibility.engine_tested in the App's app.yaml (see engine-tested.mjs).
 *
 * Environment (all optional; defaults shown in README.md):
 *   ENGINE_CWD      - Engine checkout (default /home/pi/idea/agents/agent-engine-dev)
 *   ENGINE_BIN      - path to engine dist/src/index.js (default $ENGINE_CWD/dist/src/index.js)
 *   ENGINE_MODULES  - Engine node_modules (default $ENGINE_CWD/node_modules)
 ...
 *   APP_DIR         - App repo checkout whose app.yaml gets engine_tested
 *                     (default /home/pi/idea/agents/agent-app-dev/<appName>)
 *   RECORD_ENGINE_TESTED - set to 0 to skip writing app.yaml on a passing run
 */
...
import { recordEngineTested } from './engine-tested.mjs';
...
const ENGINE_BIN = process.env.ENGINE_BIN ||
  `${ENGINE_CWD}/dist/src/index.js`;

const ENGINE_MODULES = process.env.ENGINE_MODULES ||
  `${ENGINE_CWD}/node_modules`;

// App repos nest under the agent-app-dev workspace on the Pi.
const APPS_ROOT = '/home/pi/idea/agents/agent-app-dev';
 ...
 * @param {string}   [opts.appName]    - App repo name, e.g. "app-milkwise" (locates app.yaml)
 * @param {string}   [opts.appDir]     - App repo checkout; overrides APP_DIR / default
 * @param {Function} opts.testFn       - async (port) => ({ passed, failed }) — the actual test suite
 * @returns {Promise<{passed:number, failed:number, ok:boolean}>}
 */
export async function runWithFixture({ device, fixturePath, instanceName, port, appName, appDir, testFn }) {
  let result = null;
  ...
  try {
    ...
    result = await testFn(effectivePort);
  ...
  // ── 9. Summary from actual assertion counts ─────────────────────────────
  const passed = Number.isInteger(result?.passed) ? result.passed : 0;
  const failed = Number.isInteger(result?.failed) ? result.failed : 0;
  const reported = Number.isInteger(result?.passed) && Number.isInteger(result?.failed);
  const ok = reported && failed === 0 && passed > 0;

  if (!reported) {
    log('❌ testFn did not return { passed, failed } — treating run as FAILED');
  } else if (ok) {
    log(`✅ All ${passed} assertions passed`);
  } else {
    log(`❌ ${failed} of ${passed + failed} assertions failed`);
  }

  if (!ok) {
    process.exitCode = 1;   // never write app.yaml on failure
    return { passed, failed, ok };
  }

  // ── 10. Record engine_tested in the App's app.yaml (passing runs only) ──
  if (process.env.RECORD_ENGINE_TESTED === '0') {
    log('RECORD_ENGINE_TESTED=0 — not recording engine_tested');
  } else {
    const dir = appDir || process.env.APP_DIR || (appName ? `${APPS_ROOT}/${appName}` : null);
    if (!dir) {
      log('No appName/APP_DIR given — engine_tested not recorded');
    } else {
      try {
        recordEngineTested({ appDir: dir, engineCwd: ENGINE_CWD, log });
      } catch (e) {
        log(`WARNING: could not record engine_tested in ${dir}/app.yaml: ${e.message}`);
      }
    }
  }
  return { passed, failed, ok };
